...
export default function MusicCardPlaylist({artistname, title, image}) {
  return (
    <Box  width={"100%"} borderRadius={10} padding={2}  transition={{_hover:"all ease-in-out 500ms"}} _hover={{backgroundColor:"#f6f6f648"}} display={"flex"} gap={2} alignItems={"center"}>
                      <Image alt='piccover' src={image} borderRadius={10} style={{width:50, height:50}}/>
                      <VStack alignItems={"flex-start"} gap={1} flex={1}>
                        <Heading color={"white"} lineHeight={1} fontSize={14}>{artistname}</Heading>
